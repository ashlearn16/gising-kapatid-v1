...
class OverlayView(context: Context?, attrs: AttributeSet?) :
    ...
    private var drowsyCounter: Int = 0
...
//        boxPaint.color = ContextCompat.getColor(context!!, R.color.mp_primary)
    ...
    override fun draw(canvas: Canvas) {
        ...
        results?.detections()?.map {
            ...
            // Change the color of the bounding box and the background color of text based on the detected class
            when(category.categoryName()) {
                "awake" -> {

                    if (drowsyCounter >= 0) {
                        Log.d("Awake", drowsyCounter.toString())
                        drowsyCounter--
                    }

                    if (drowsyCounter == 0 && isAlarmPlaying) {
                        // Stop playing the alarm MP3
                        stopAlarm()
                        isAlarmPlaying = false
                    }

                    boxPaint.color = ContextCompat.getColor(context!!, R.color.mp_primary)
                    textBackgroundPaint.color = Color.argb(100, 76, 175, 80)
                }

                "drowsy" -> {

                    if (drowsyCounter <= 5) {
//                        Toast.makeText(context, drowsyCounter, Toast.LENGTH_LONG).show()
                        Log.d("Drowsy", drowsyCounter.toString())
                        drowsyCounter++
                    }

                    if (drowsyCounter >= 5 && !isAlarmPlaying) {
                        // Start playing the alarm MP3
                        playAlarm()
                        isAlarmPlaying = true
                    }

                    boxPaint.color = ContextCompat.getColor(context!!, R.color.mp_drowsy)
                    textBackgroundPaint.color = Color.argb(100, 244, 67, 54)
    ...
    private fun stopAlarm() {
        try {
            mediaPlayer?.let {
                if (it.isPlaying) {
                    it.stop() // Stop playing the alarm sound
